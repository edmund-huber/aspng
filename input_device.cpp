...
std::string InputDevice::prefix(void) {
    return "input";
}

// A valid InputDevice must be empty (black).
bool InputDevice::sub_parse(AspngSurface *surface, int32_t min_x, int32_t min_y, int32_t max_x, int32_t max_y, std::string param) {
    if (param != "") {
        return false;
    }
